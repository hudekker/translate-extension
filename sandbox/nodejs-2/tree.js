...
// Function to authenticate and get the Google Drive client
    ...
    if (files && files.length > 0) {
      ...
      await Promise.all(files.map(async (file) => {
        if (file.mimeType === 'application/vnd.google-apps.folder') {
          ...
          let displayFolderName = indentation(depth, file.name);
          // console.log(`${depth}\t${displayFolderName}\t${file.id}`);

          // Add this line to log the parent ID
          // console.log('Parent ID:', folderId);

          // Recursive call only if parentId is not null
          // if (file.id) {
          foldersraw.push({
            id: file.id,
            name: file.name,
...
async function main() {
  try {
    ...
    const tree2 = tree.map(({ name, id, depth }) => ({ depth, name, id }));
    console.table(tree2)
  } catch (error) {
